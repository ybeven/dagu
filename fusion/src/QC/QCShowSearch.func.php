<?php
function QCShowSearch($notice = null,$page=0){
	global $smarty;
	needLogin();
	...
	$task = new Task;
	$sql = "select * from `task` WHERE plan_id in (select id from plan where ((operate_time LIKE '{$date1}%' OR operate_time LIKE '{$date2}%') OR (operate_time > '{$date1}%' AND operate_time < '{$date2}%' )) AND truck_number LIKE '%{$truck}%')  AND task.customer_id in (select id from `customer` WHERE customer.name LIKE '%{$company}%')";
	if($oilModelId != null)
	{
		$sql .= "  AND task.oil_model_id = '{$oilModelId}'";
	}
	if($state1 != 1)
	{
	...
	//if($oilModelId != null)
	//{
	//	$sql+= " AND task.oilModelId = '{$oilModelId}'";
	//}
	
	
	//$sql = "select * from `task`,`customer` WHERE  task.final_operate_time LIKE '{$date}%' AND customer.id = task.customer_id AND customer.name LIKE '%{$company}%'";
