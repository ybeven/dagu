<?php
function SearchPreDataDetails($tag,$notice = null,$page=0){
	global $smarty;
	needLogin();
	...
	$examcardnum=$_POST["examcardnum"];
	// print_r("skflsafjsf");
	// print_r($examcardnum);
	if ($examcardnum!=null) {
		$sql1 =  "SELECT * FROM shanxijianshe where examcardnum=$examcardnum";
    ...
    $crumb->visitNewPage("根据准考证号查询" , "predata/SearchPreDataDetails" , true);
	$smarty->setTitle('查看准考证成绩');
	$smarty->displayMother('predata/SearchPreDataDetails.tpl');
}
?>