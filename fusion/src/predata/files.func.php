...
function files($mineid){
	...
	$flag=0;
		...
		$fileName = mb_convert_encoding($_FILES['file1']['name'],"GBK","auto");//$_FILES['file1']['name'];
		...
		$fileName = mb_convert_encoding($_FILES['file2']['name'],"GBK","auto");//$_FILES['file1']['name'];
		...
		$fileName = mb_convert_encoding($_FILES['file3']['name'],"GBK","auto");//$_FILES['file1']['name'];
...
if ($_FILES["file4"]["error"]==0)
		...
		$fileName = mb_convert_encoding($_FILES['file4']['name'],"GBK","auto");//$_FILES['file1']['name'];
		...
		if ($_FILES["file5"]["error"]==0)
  		{
			$flag=1;
		$upload_name=$_FILES['file5']['name'];
		$fileName = mb_convert_encoding($_FILES['file5']['name'],"GBK","auto");//$_FILES['file1']['name'];
		...
		if ($_FILES["file6"]["error"]==0)
  		{
			$flag=1;
		$upload_name=$_FILES['file6']['name'];
		$fileName = mb_convert_encoding($_FILES['file6']['name'],"GBK","auto");//$_FILES['file1']['name'];
		...
		if ($_FILES["file7"]["error"]==0)
  		{
			$flag=1;
		$upload_name=$_FILES['file7']['name'];
		$fileName = mb_convert_encoding($_FILES['file7']['name'],"GBK","auto");//$_FILES['file1']['name'];
		...
		$data->fileName = $name;
		$data->fileData = $uploadPath;
		$data->type = $_FILES["file7"]["type"];  
		$sql = $data->add();
        set_time_limit(30); //恢复缺省超时设置
		}

		if($flag==1)
			http302('/minedata/AddFiles/'.$mineid.'/'.urlencode('添加成功！'));  
		else
			http302('/minedata/AddFiles/'.$mineid.'/'.urlencode('添加失败！'));
