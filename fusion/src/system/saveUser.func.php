...
function performAdd($savedUserInfo , &$session){
	...
	if(!checkNecessary($postArray , $savedUserInfo , $item)){
		http302("/system/manageUser/auto/".urlencode($item."未填写，请仔细检查"));
		return;
	}
...
POSTDEF;
	...
	if(!checkFormat($savedUserInfo , $postDef , $item)){
		http302("/system/manageUser/auto/".urlencode($postArray[$item]."格式不正确，请仔细检查"));
		return;
	}
	...
	try{
		...
		http302("/system/manageUser/auto/".urlencode("用户添加成功"));
		return;
	}
	catch(PDOException $ex){
		if($ex->getCode() == MYSQL_DUPLICATE_KEY){ //duplicate key
			http302("/system/manageUser/auto/".urlencode($savedUserInfo['name']."已经存在！"));
			return;
		}
		else
			throw $ex;
...
POSTDEF;
	...
	try{
		...
		http302("/system/manageUser/auto/".urlencode("用户信息更新成功"));
		return;
	}
	catch(PDOException $ex){
		throw $ex;	
