<?php		
function Add($orename,$lvtype,$notice=null){
    global $smarty;
	needLogin();
	...
	// }
	//插入新的
	if($_POST[newsubclass1]!="")
	{
	$sql = " insert into `{$neirongbiao}` (kuangzhong,subclass1,subclass2,subclass3,subclass4,subclass5,subclass6,standardvalues) values('{$orename}','{$_POST[newsubclass1]}','{$_POST[newsubclass2]}','{$_POST[newsubclass3]}','{$_POST[newsubclass4]}','{$_POST[newsubclass5]}','{$_POST[newsubclass6]}','{$_POST[newstandardvalues]}') ";
	mysql_query($sql,$db);
	}
	//返回List页面
// $MClass = new Oretype;
...
// $sql = "SELECT kuangzhong,subclass_name1,subclass_name2,subclass_name3,subclass_name4,subclass_name5,subclass_name6
    ...
    // $crumb->visitNewPage("三率标准" , "/threepersent/List" , true);	
	// $smarty->setTitle('三率标准');
	// $smarty->displayMother('threepersent/List.tpl');
	http302('/system/Searchthreepersent/'.$orename.'/'.$lvtype.'/');
}
?>