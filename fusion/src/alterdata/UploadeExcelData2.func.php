...
function UploadeExcelData2($notice=null){
	global $smarty;
	needLogin();
...
/********Excel update to data********/
	...
	if($_POST['leadExcel'] == "true")
	{
		...
		{			
			...
			$examcardnum = $declaredata[$tmp]['2'];
			// $declaredata[$tmp]['3'] = $currentSheet[$Count]->getCell($address2)->getValue();
			//zhunkaozhenghao
			$course = $declaredata[$tmp]['3'];
			$sql = "select * from shanxijianshe where pici=$pici and examcardnum='$examcardnum'";
			// print_r('sql:'.$sql);
			...
			foreach($dataTemp as $i)
			{
		        ...
		        $ShanxiTemp->examcardnum .= $examcardnum;
    ...
    $crumb->visitNewPage("需要修改成绩-按照准考证号" , "/alterdata/AddAlterData2" , true);	    
	...
	$smarty->displaymother('alterdata/AddAlterData2.tpl');
}
?>