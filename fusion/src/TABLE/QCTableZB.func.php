<?php
function QCTableZB($notice = null,$page=0){
	global $smarty;
	needLogin();
	...
	$workId = 1;
	$task = new Task;
	$sql = "select * from `task` WHERE  final_operate_time LIKE '{$planDate}%' AND final_work_id = $workId ";
	...
	///////////////////////////////////////////////////////////////////////////////////Excel部分
        $fileName = ''.$planDate.' '.$workID[0]->name.'.xls';
	...
	$objActiveSheet->setCellValue('A2', ' '.$planDate.'  '.$workID[0]->name);
	$objActiveSheet->getColumnDimension('A')->setWidth(25);
        ...
        if($taskArray != null)
	{
		  ...
		  if($childID != null)
		  {
			foreach($childID as $n)
		        {
			        $task22 = new Task;
	                        $sql22 = "select * from `task` WHERE  final_operate_time LIKE '{$date}%' AND final_work_id = $workId AND oil_model_id = '{$n->id}' ";
	...
	$smarty->displayMother('TABLE/QCTableZB.tpl');
}
?>