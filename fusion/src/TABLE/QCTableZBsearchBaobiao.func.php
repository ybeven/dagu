<?php
function QCTableZBsearchBaobiao($notice = null){
	global $smarty;
	needLogin();
	...
	$task = new Task;
	$sql = "select * from `task` WHERE  final_operate_time LIKE '{$date}%'";
        ...
        if($taskArray != null)
	{
		  ...
		  if($childID != null)
		  {
			foreach($childID as $n)
		        {
			        $task22 = new Task;
	                        $sql22 = "select * from `task` WHERE  final_operate_time LIKE '{$date}%'  AND oil_model_id = '{$n->id}' ";
