<?php
function QCTableJS($notice = null,$page=0){
	global $smarty;
	needLogin();
	...
	$task = new Task;
	$sql = "select * from `task` WHERE  final_operate_time LIKE '{$date}%' AND final_work_id = '{$workID[0]->id}' ";
	...
	$objActiveSheet->setCellValue('A2', ' '.$date.'  '.$workID[0]->name);
	$objActiveSheet->getColumnDimension('A')->setWidth(25);
        ...
        if($taskArray != null)
	{
		  ...
		  if($childID != null)
		  {
			foreach($childID as $n)
		        {
			        $task22 = new Task;
	                        $sql22 = "select * from `task` WHERE  final_operate_time LIKE '{$date}%' AND final_work_id = '{$workID[0]->id}' AND oil_model_id = '{$n->id}' ";
