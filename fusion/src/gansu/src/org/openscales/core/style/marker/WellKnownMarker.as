package org.openscales.core.style.marker
{
	...
	public class WellKnownMarker extends Marker
	{
		...
		public function WellKnownMarker(wellKnownName:String = WKN_SQUARE, fill:SolidFill = null, stroke:Stroke = null, size:Number = 6, opacity:Number = 1, rotation:Number = 0)
		{
		...
		override public function get instance():DisplayObject{
		...
		override protected function generateGraphic():DisplayObject{
			...
			switch (this.wellKnownName) {
				...
				case WellKnownMarker.WKN_SQUARE:  {
					shape.graphics.drawRect( -(this.size / 2), -(this.size / 2), this.size, this.size);
					break;
				}
				case WellKnownMarker.WKN_CIRCLE:  {
					shape.graphics.drawCircle(0, 0, this.size / 2);
					break;
				}
				case WellKnownMarker.WKN_TRIANGLE:  {
					shape.graphics.moveTo(0,  -(this.size / 2));
					shape.graphics.lineTo(this.size / 2, this.size / 2);
					shape.graphics.lineTo(-this.size / 2, this.size / 2);
					shape.graphics.lineTo(0, (this.size / 2));
					break;
				}
