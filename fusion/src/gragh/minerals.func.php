<?php
function minerals($page=0 , $notice=null){
    global $smarty;
...
if($_POST[index]==0)$graph->title->Set("甘肃全省 绿色矿山数量统计（煤矿）");
if($_POST[index]==1) $graph->title->Set("绿色矿山矿区面积统计");

$x=array("2007年","2008年","2009年","2010年","2011年","2012年","2013年");
...
if($_POST[mineral]==NULL) $_POST[mineral]='铝矿+铁矿';
//if($_POST[mineral]!='煤') exit;
//echo $_POST[mineral];
$minerals_post=explode("+",$_POST[mineral]);
$mineral_number=count($minerals_post);
//echo $mineral_number;
//print_r($minerals_post);
//if($_POST[mineral]!=NULL) $mineral=substr($_POST[mineral],0,19).'...';;
if($_POST[mineral]!=NULL) $mineral_to_tpl=$_POST[mineral];//mb_substr($_POST[mineral],0,4,'UTF-8');
//echo $mineral;
//if($_POST[mineral]!='煤') exit;
...
//if($_POST[positionx]!="时间"){  //统一选择6个矿种
  // if($x_number!=6) echo "<script>alert('为图表的统一样式，请选择6种矿种！');self.location='/gragh/minerals';</script>";
//}
if($xs[0]=="时间"&&$x_number!=1){  //统一选择6个矿种
    echo "<script>alert('时间和行政区划不能同时选择！');self.location='/gragh/minerals';</script>";
}
...
$dataofminerals=array();
for($eachmineral=0;$eachmineral<$mineral_number;$eachmineral++){
	//echo "qiujinhan";
	$mineral=$minerals_post[$eachmineral];  //这里找出了矿种
	//echo $mineral;
...
if($_POST[index]==1||$_POST[index]==NULL)           
{
	...
	{
		$datay[$i]=0;
		$sql="SELECT * FROM basedata,environment,ore where basedata_jiqizhi=$year and ore.orename='$mineral' and basedata.mine_id=ore.mine_id and ore.oretype='主矿种' and basedata.mine_id=environment.mine_id";
...
if($_POST[positionx]!="时间")	//x轴为矿山类型的
	{                             //兰州市先做一个，工业总产值的柱状图，条件是金属矿种的。
		...
		for($mineralindex=0;$mineralindex<$x_number;$mineralindex++)  //铁，铜，金，锰，钛，铝的值,把每一种矿的对应值加起来。
		{
			...
			$sql="SELECT * FROM basedata,ore,environment where basedata_divisions_shi='$basedata_mineral_main' and ore.orename='$mineral' and basedata.mine_id=ore.mine_id and ore.oretype='主矿种' and basedata.mine_id=environment.mine_id";
...
if($_POST[index]==2)                         
{
	...
	{
		$datay[$i]=0;
		$sql="SELECT count(*) as ct FROM basedata,ore where basedata_jiqizhi=$year and ore.orename='$mineral' and basedata.mine_id=ore.mine_id and ore.oretype='主矿种'";
...
if($_POST[positionx]!="时间")	//x轴为矿山类型的
	{                             //兰州市先做一个，工业总产值的柱状图，条件是金属矿种的。
		...
		for($mineralindex=0;$mineralindex<$x_number;$mineralindex++)  //铁，铜，金，锰，钛，铝的值,把每一种矿的对应值加起来。
		{
			...
			$datay[$mineralindexin]=0;
			$basedata_mineral_main=$xs[$mineralindex];
			$sql="SELECT count(*) as ct FROM basedata,ore where ore.orename='$mineral' and basedata_divisions_shi='$basedata_mineral_main' and basedata.mine_id=ore.mine_id and ore.oretype='主矿种'";
...
if($_POST[index]==3)            
{
	...
	if($_POST[positionx]=="时间"&&$x_number==1)  //x轴为时间的
	{
		...
		{
			$datay[$i]=0;
			$sql="SELECT * FROM basedata,ore where basedata_jiqizhi=$year and ore.orename='$mineral' and basedata.mine_id=ore.mine_id and ore.oretype='主矿种'";
			//if($city=='甘肃全省') $sql="SELECT * FROM basedata where basedata_buildtime=$year";
			$result = mysql_query($sql);
			while($row=mysql_fetch_assoc($result))  //把每一年的加起来
			{
	...
	{                             //兰州市先做一个，工业总产值的柱状图，条件是金属矿种的。
		...
		for($mineralindex=0;$mineralindex<$x_number;$mineralindex++)  //铁，铜，金，锰，钛，铝的值,把每一种矿的对应值加起来。
		{
			...
			$datay[$mineralindexin]=0;
			$basedata_mineral_main=$xs[$mineralindex];
			$sql="SELECT * FROM basedata,ore where basedata_divisions_shi='$basedata_mineral_main' and ore.orename='$mineral' and basedata.mine_id=ore.mine_id and ore.oretype='主矿种'";
			//if($city=='甘肃全省') $sql="SELECT,ore * FROM basedata where basedata_mineral_main='$basedata_mineral_main'";
			$result = mysql_query($sql);
			while($row=mysql_fetch_assoc($result))  //把每一年的加起来
			{
				$datay[$mineralindexin]=$datay[$mineralindexin]+$row[$index];	
...
if($_POST[index]==6)                          
{
	...
	{
		$datay[$i]=0;
		$sql="SELECT avg(energy_energy) as av FROM basedata,energy,ore where basedata_jiqizhi=$year and ore.orename='$mineral' and basedata.mine_id=ore.mine_id and ore.oretype='主矿种' and energy.mine_id=basedata.mine_id";
  ...
  if($_POST[positionx]!="时间")	//x轴为矿山类型的
	{                             //兰州市先做一个，工业总产值的柱状图，条件是金属矿种的。
		...
		for($mineralindex=0;$mineralindex<$x_number;$mineralindex++)  //铁，铜，金，锰，钛，铝的值,把每一种矿的对应值加起来。
		{
			...
			$datay[$mineralindexin]=0;
			$basedata_mineral_main=$xs[$mineralindex];
			$sql="SELECT avg(energy_energy) as av FROM basedata,energy,ore where basedata_divisions_shi='$basedata_mineral_main' and energy.mine_id=basedata.mine_id and ore.orename='$mineral' and basedata.mine_id=ore.mine_id and ore.oretype='主矿种'";
...
if($_POST[index]==7)                        
{
	...
	{
		$datay[$i]=0;
		$sql="SELECT sum(reclamation_rate_have) as av FROM basedata,reclamation,ore where basedata_jiqizhi=$year and ore.orename='$mineral' and basedata.mine_id=ore.mine_id and ore.oretype='主矿种' and reclamation.mine_id=basedata.mine_id";
 ...
 if($_POST[positionx]!="时间")	//x轴为矿山类型的
	{                             //兰州市先做一个，工业总产值的柱状图，条件是金属矿种的。
		...
		for($mineralindex=0;$mineralindex<$x_number;$mineralindex++)  //铁，铜，金，锰，钛，铝的值,把每一种矿的对应值加起来。
		{
			...
			$datay[$mineralindexin]=0;
			$basedata_mineral_main=$xs[$mineralindex];
			$sql="SELECT sum(reclamation_rate_have) as av FROM basedata,reclamation,ore where basedata_divisions_shi='$basedata_mineral_main' and ore.orename='$mineral' and basedata.mine_id=ore.mine_id and ore.oretype='主矿种' and reclamation.mine_id=basedata.mine_id";
...
if($_POST[index]==9)                        
{
	...
	{
		$datay[$i]=0;
		$sql="SELECT sum(science_rate_scimoney) as av FROM basedata,science,ore where basedata_jiqizhi=$year and ore.orename='$mineral' and basedata.mine_id=ore.mine_id and ore.oretype='主矿种' and science.mine_id=basedata.mine_id";
		//if($city=='甘肃全省')  $sql="SELECT * FROM basedata where basedata_buildtime=$year";
		$result = mysql_query($sql);
		while($row=mysql_fetch_assoc($result))  //把每一年的加起来
		{
 ...
 if($_POST[positionx]!="时间")	//x轴为矿山类型的
	{                             //兰州市先做一个，工业总产值的柱状图，条件是金属矿种的。
		...
		for($mineralindex=0;$mineralindex<$x_number;$mineralindex++)  //铁，铜，金，锰，钛，铝的值,把每一种矿的对应值加起来。
		{
			...
			$datay[$mineralindexin]=0;
			$basedata_mineral_main=$xs[$mineralindex];
			$sql="SELECT sum(science_rate_scimoney) as av FROM basedata,science,ore where basedata_divisions_shi='$basedata_mineral_main' and ore.orename='$mineral' and basedata.mine_id=ore.mine_id and ore.oretype='主矿种' and science.mine_id=basedata.mine_id";
...
if($_POST[index]==10)                          
{
	...
	{
		$datay[$i]=0;
		$sql="SELECT sum(project_Cost) as av FROM basedata,project,ore where basedata_jiqizhi=$year and ore.orename='$mineral' and basedata.mine_id=ore.mine_id and ore.oretype='主矿种' and project.mine_id=basedata.mine_id";
	   //if($city=='甘肃全省') $sql="SELECT * FROM basedata where basedata_buildtime=$year";
		$result = mysql_query($sql);
		while($row=mysql_fetch_assoc($result))  //把每一年的加起来
		{
 ...
 if($_POST[positionx]!="时间")	//x轴为矿山类型的
	{                             //兰州市先做一个，工业总产值的柱状图，条件是金属矿种的。
		...
		for($mineralindex=0;$mineralindex<$x_number;$mineralindex++)  //铁，铜，金，锰，钛，铝的值,把每一种矿的对应值加起来。
		{
			...
			$sql="SELECT sum(project_Cost) as av FROM basedata,project,ore where basedata_divisions_shi='$basedata_mineral_main' and ore.orename='$mineral' and basedata.mine_id=ore.mine_id and ore.oretype='主矿种' and project.mine_id=basedata.mine_id";
			//if($city=='甘肃全省') $sql="SELECT avg(environment_rate_green) as av FROM basedata,environment where environment.mine_id=basedata.mine_id and basedata_mineral_main='$basedata_mineral_main'";
			$result = mysql_query($sql);
			$j=0;
			while($row=mysql_fetch_assoc($result))  //把每一年的加起来
...
$dataofmineral[$eachmineral]=$datay;
//var_dump($datay);

} //大的for循环结束

//var_dump($dataofmineral);
//var_dump($dataofmineral['0']);
...
for($i=0;$i<$mineral_number;$i++){
    //$datay=$dataofCitys[$i];
	if($_POST[positionx]=="时间"||$_POST[positionx]==NULL){
	for($j=0;$j<6;$j++){
		$datay[$j]=$dataofmineral[$i][$j];
	}
	}
	else{
	...
	//$tuli=array('规划值','实际值');
	$$bplot[$i]->SetLegend($minerals_post[$i]);
}
...
//echo $city_number;
if($mineral_number==1)  $gbarplot=new AccBarPlot(array($$bplot[0]));
if($mineral_number==2) {$gbarplot=new AccBarPlot(array($$bplot[0],$$bplot[1]));}
if($mineral_number==3) {$gbarplot=new AccBarPlot(array($$bplot[0],$$bplot[1],$$bplot[2]));}
if($mineral_number==4) {$gbarplot=new AccBarPlot(array($$bplot[0],$$bplot[1],$$bplot[2],$$bplot[3]));}
if($mineral_number==5) {$gbarplot=new AccBarPlot(array($$bplot[0],$$bplot[1],$$bplot[2],$$bplot[3],$$bplot[4]));}
if($mineral_number==6) {$gbarplot=new AccBarPlot(array($$bplot[0],$$bplot[1],$$bplot[2],$$bplot[3],$$bplot[4],$$bplot[5]));}
if($mineral_number==7) {$gbarplot=new AccBarPlot(array($$bplot[0],$$bplot[1],$$bplot[2],$$bplot[3],$$bplot[4],$$bplot[5],$$bplot[6]));}
if($mineral_number==8) {$gbarplot=new AccBarPlot(array($$bplot[0],$$bplot[1],$$bplot[2],$$bplot[3],$$bplot[4],$$bplot[5],$$bplot[6],$$bplot[7]));}
if($mineral_number==9) {$gbarplot=new AccBarPlot(array($$bplot[0],$$bplot[1],$$bplot[2],$$bplot[3],$$bplot[4],$$bplot[5],$$bplot[6],$$bplot[7],$$bplot[8]));}
if($mineral_number==10) {$gbarplot=new AccBarPlot(array($$bplot[0],$$bplot[1],$$bplot[2],$$bplot[3],$$bplot[4],$$bplot[5],$$bplot[6],$$bplot[7],$$bplot[8],$$bplot[9]));}
if($mineral_number==11) {$gbarplot=new AccBarPlot(array($$bplot[0],$$bplot[1],$$bplot[2],$$bplot[3],$$bplot[4],$$bplot[5],$$bplot[6],$$bplot[7],$$bplot[8],$$bplot[9],$$bplot[10]));}
if($mineral_number==12) {$gbarplot=new AccBarPlot(array($$bplot[0],$$bplot[1],$$bplot[2],$$bplot[3],$$bplot[4],$$bplot[5],$$bplot[6],$$bplot[7],$$bplot[8],$$bplot[9],$$bplot[10],$$bplot[11]));}
if($mineral_number==13) {$gbarplot=new AccBarPlot(array($$bplot[0],$$bplot[1],$$bplot[2],$$bplot[3],$$bplot[4],$$bplot[5],$$bplot[6],$$bplot[7],$$bplot[8],$$bplot[9],$$bplot[10],$$bplot[11],$$bplot[12]));}
if($mineral_number==14) {$gbarplot=new AccBarPlot(array($$bplot[0],$$bplot[1],$$bplot[2],$$bplot[3],$$bplot[4],$$bplot[5],$$bplot[6],$$bplot[7],$$bplot[8],$$bplot[9],$$bplot[10],$$bplot[11],$$bplot[12],$$bplot[13]));}
...
{
if($mineral_number>1) echo "<script>alert('折线图只能选择一种矿种!');self.location='/gragh/region';</script>";
...
if($mineral_number==1)  $gbarplot=new AccLinePlot(array($$bplot[0]));
if($mineral_number==2) {$gbarplot=new AccLinePlot(array($$bplot[0],$$bplot[1]));}
if($mineral_number==3) {$gbarplot=new AccLinePlot(array($$bplot[0],$$bplot[1],$$bplot[2]));}
if($mineral_number==4) {$gbarplot=new AccLinePlot(array($$bplot[0],$$bplot[1],$$bplot[2],$$bplot[3]));}
if($mineral_number==5) {$gbarplot=new AccLinePlot(array($$bplot[0],$$bplot[1],$$bplot[2],$$bplot[3],$$bplot[4]));}
if($mineral_number==6) {$gbarplot=new AccLinePlot(array($$bplot[0],$$bplot[1],$$bplot[2],$$bplot[3],$$bplot[4],$$bplot[5]));}
if($mineral_number==7) {$gbarplot=new AccLinePlot(array($$bplot[0],$$bplot[1],$$bplot[2],$$bplot[3],$$bplot[4],$$bplot[5],$$bplot[6]));}
if($mineral_number==8) {$gbarplot=new AccLinePlot(array($$bplot[0],$$bplot[1],$$bplot[2],$$bplot[3],$$bplot[4],$$bplot[5],$$bplot[6],$$bplot[7]));}
if($mineral_number==9) {$gbarplot=new AccLinePlot(array($$bplot[0],$$bplot[1],$$bplot[2],$$bplot[3],$$bplot[4],$$bplot[5],$$bplot[6],$$bplot[7],$$bplot[8]));}
if($mineral_number==10) {$gbarplot=new AccLinePlot(array($$bplot[0],$$bplot[1],$$bplot[2],$$bplot[3],$$bplot[4],$$bplot[5],$$bplot[6],$$bplot[7],$$bplot[8],$$bplot[9]));}
if($mineral_number==11) {$gbarplot=new AccLinePlot(array($$bplot[0],$$bplot[1],$$bplot[2],$$bplot[3],$$bplot[4],$$bplot[5],$$bplot[6],$$bplot[7],$$bplot[8],$$bplot[9],$$bplot[10]));}
if($mineral_number==12) {$gbarplot=new AccLinePlot(array($$bplot[0],$$bplot[1],$$bplot[2],$$bplot[3],$$bplot[4],$$bplot[5],$$bplot[6],$$bplot[7],$$bplot[8],$$bplot[9],$$bplot[10],$$bplot[11]));}
if($mineral_number==13) {$gbarplot=new AccLinePlot(array($$bplot[0],$$bplot[1],$$bplot[2],$$bplot[3],$$bplot[4],$$bplot[5],$$bplot[6],$$bplot[7],$$bplot[8],$$bplot[9],$$bplot[10],$$bplot[11],$$bplot[12]));}
if($mineral_number==14) {$gbarplot=new AccLinePlot(array($$bplot[0],$$bplot[1],$$bplot[2],$$bplot[3],$$bplot[4],$$bplot[5],$$bplot[6],$$bplot[7],$$bplot[8],$$bplot[9],$$bplot[10],$$bplot[11],$$bplot[12],$$bplot[13]));}
...
if($all==0) echo "<script>alert('您要查询的数据位空不能生产饼状图!');self.location='/gragh/minerals';</script>";
if($mineral_number>1) echo "<script>alert('饼状图只能选择一种矿种!');self.location='/gragh/minerals';</script>";
...
$picture='/graph1.png';
	...
	$smarty->assign('mineral', $mineral_to_tpl);
    ...
    $smarty->displayMother('gragh/minerals.tpl');
}
?>